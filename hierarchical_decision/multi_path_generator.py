from environment.env_horizon.dynamics_and_models import ReferencePath
import numpy as np
...
class MultiPathGenerator(object):
    def __init__(self, path_num, ref_index=3, expected_v=None):
        # state: [v_x, v_y, r, x, y, phi(°)]
        self.path_num = path_num  # number of trajectories
    ...
    def generate_path(self, task):
        ...
        for path_index in range(self.path_num):
            ref = ReferencePath(task, expected_v=self.exp_v)
...
class StaticTrajectoryGenerator_origin(object):
    def __init__(self, mode, ref_index=3, v_light=0, expected_v=None):
        # state: [v_x, v_y, r, x, y, phi(°)]
        self.mode = mode
