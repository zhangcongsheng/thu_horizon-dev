...
class UpdateThread(threading.Thread):
    ...
    def __init__(self, workers, evaluator, args, optimizer_stats):
        ...
        self.writer = SummaryWriter(self.log_dir + '/optimizer')
    ...
    def step(self):  # thread这个类每次都默认执行的函数，更新主神经网络的参数
        ...
        with self.grad_queue_get_timer:
            try:
                ...
                if self.grad_reuse < self.args.grads_max_reuse:
                    ...
                    self.grad, self.learner_stats = self.inqueue.get(timeout=300)
                    # Note 30 s is not enough for restart worker
                    self.grad_reuse = 0
        # apply grad
        with self.grad_apply_timer:
            try:
                ...
                self.grad = [np.zeros_like(grad) for grad in self.grad]
        ...
        if self.iteration % self.args.log_interval == 0:
            ...
            for key, val in self.learner_stats.items():
                if not isinstance(val, list):
                    self.writer.add_scalar('optimizer/learner_stats/scalar/{}'.format(key), val,
                                           global_step=self.iteration)
                else:
                    assert isinstance(val, list)
                    for i, v in enumerate(val):
                        self.writer.add_scalar('optimizer/learner_stats/list/{}/{}'.format(key, i), v,
                                               global_step=self.iteration)
            for key, val in self.optimizer_stats.items():
                self.writer.add_scalar('optimizer/{}'.format(key), val, global_step=self.iteration)
            self.writer.flush()
...
class OffPolicyAsyncOptimizer(object):
    def __init__(self, workers, learners, replay_buffers, evaluator, args):
        ...
        self.workers_id_num = 1 + len(self.workers["remote_workers"])
        ...
        self.update_thread = UpdateThread(self.workers, self.evaluator, self.args,
                                          self.stats)
        self.update_thread.start()  # 只要start()之后，self.update_thread.step()就会一直跑
        ...
        self._set_workers()  # 把local worker的参数设置给remote wokers

        # fill buffer to replay starts
        logger.info('start filling the replay')  # 保证所有的buffer都有足够的数据（3000）
        ...
        self.replay_tasks = TaskPool()  # 对ray的接口进行封装，处理任务冲突
        self._set_buffers()
    ...
    def get_stats(self):
        ...
        # self.stats.update(ray.get(self.workers['remote_workers'][0].get_fill_num.remote()))  # SLOW!!!
        return self.stats
    ...
    # def restart_a_worker(self):
    #     logger.warning("==================== try to restart a worker ===================================")
    #     worker = ray.remote(num_cpus=1)(OffPolicyWorker).remote(
    #         Policy4Horizon,
    #         self.args.env_id,
    #         self.args,
    #         self.workers_id_num + 1
    #     )
    #     self.workers_id_num += 1
    #     return worker
    ...
    def step(self):
        ...
        with self.timers['sampling_timer']:
            for worker, objID in self.sample_tasks.completed():
                sample_batch, count = ray.get(objID)
                random.choice(self.replay_buffers).add_batch.remote(sample_batch)  # 选一个buffer，并添加数据
                self.num_sampled_steps += count
                ...
                if self.steps_since_update[worker] >= self.max_weight_sync_delay:
                    policy_state_dict = []
                    for key in self.local_worker.policy_with_value.policy.state_dict():  # maybe useless
                        policy_state_dict.append(self.local_worker.policy_with_value.policy.state_dict()[key])
                    judge_is_nan(policy_state_dict)
                    if weights is None:
        ...
        with self.timers["replay_timer"]:
            for rb, replay in self.replay_tasks.completed():
                ...
                if self.learner_queue.full():
                    ...
                    self.learner_queue.put((rb, samples))  # TODO: 有必要把整个rb放进队列么？
    ...
    def step(self):
        ...
        with self.timers['grad_apply_timer']:
            try:
                ...
                grads = [np.zeros_like(grad) for grad in grads]
                logger.info('Grad is nan!, zero it')
