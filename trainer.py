...
class Trainer(object):
    def __init__(self, policy_cls, worker_cls, learner_cls, buffer_cls, optimizer_cls, evaluator_cls, args):
        ...
        if self.args.optimizer_type.startswith('SingleProcess'):
            ...
            if self.args.off_policy:  # off-policy算法
                ...
                self.learners = [ray.remote(num_cpus=1)(learner_cls).remote(policy_cls, args, i + 1)
                                 for i in range(self.args.num_learners)]
    ...
    def sync_remote_workers(self):
        weights = ray.put(self.local_worker.get_weights())  # 放到公共内存里，分享快一点
